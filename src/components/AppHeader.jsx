  ...
  return (
    <header className={`sticky top-0 z-40 flex w-full items-center justify-between bg-background/95 px-margin-mobile py-base backdrop-blur ${className}`}>
      <div className="flex min-w-0 items-center gap-gutter">
        ...
        {showProfile ? (
          <div className="flex h-10 w-10 items-center justify-center rounded-full border border-outline-variant bg-primary-fixed text-on-primary-fixed">
            <Icon fill>school</Icon>
          </div>
        ) : null}
...
export function TextHeader({ title, leftLabel = "취소", rightLabel, onRight, rightDisabled = false }) {
  return (
    <header className="sticky top-0 z-40 flex h-16 w-full items-center justify-between border-b border-outline-variant/40 bg-surface px-margin-mobile">
      <Link to="/" className="rounded-lg p-2 text-on-surface-variant transition-colors hover:bg-surface-container-low active:scale-95">
      ...
      <button
        ...
        className="rounded-lg px-4 py-2 text-title-md font-title-md font-bold text-primary transition-colors hover:bg-primary-container/10 active:scale-95 disabled:cursor-not-allowed disabled:opacity-40"
      >
        {rightLabel}
      </button>
    </header>
