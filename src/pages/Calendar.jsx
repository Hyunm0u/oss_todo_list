...
export default function Calendar() {
  ...
  const deleteRecurringTask = (task) => {
    const rootId = task.parentId || task.id;

    const hasSeries = tasks.some(
      (item) =>
        item.id === rootId ||
    ...
    if (hasSeries) {
      const deleteSeries = window.confirm(
        "확인을 누르면 모든 반복 작업을 삭제합니다.\n취소를 누르면 이번 작업만 삭제합니다."
      );
        ...
        <section className="space-y-stack-md">
          ...
          {selectedEvents.length || selectedTasks.length ? (
            <div className="grid grid-cols-1 gap-stack-md md:grid-cols-2">
              ...
              {selectedTasks.map((task) => <TaskCard key={task.id} task={task} onDelete={() => deleteRecurringTask(task)} />)}
            </div>
          ) : (
  ...
  return (
    <SurfaceCard className="flex gap-gutter p-stack-md">
      ...
      <div className="min-w-0 flex-1">
        ...
        <h3 className={`truncate text-body-lg font-title-md text-on-surface ${task.completed ? "line-through opacity-60" : ""}`}>
          {task.title}
          {task.recurring !== "none" && (
  ...
  return (
    <SurfaceCard className="flex gap-gutter p-stack-md">
      ...
      <div className="min-w-0 flex-1">
        ...
        <h3 className="truncate text-body-lg font-title-md text-on-surface">{event.title}{event.recurring !== "none" && (<span className="ml-1 text-xs text-on-surface-variant">🔁</span>)}</h3>
      </div>
